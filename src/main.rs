...
async fn main() -> anyhow::Result<()> {
    ...
    use fluidweb::{
        ...
        server::{
            AppState, auth, db,
            graphql::{self, MutationRoot, QueryRoot, RequestAuth},
        },
    };
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "fluidweb=info,tower_http=info".into()),
        )
        .init();
    ...
    if args.get(1).map(String::as_str) == Some("create-admin") {
        if args.len() != 6 {
            anyhow::bail!(
                "usage: fluidweb create-admin <email> <first-name> <last-name> <password>"
            );
        }
    ...
    async fn graphql_handler(
        ...
        state
            .schema
            .execute(request.into_inner().data(RequestAuth {
                user,
